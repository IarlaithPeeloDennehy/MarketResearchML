  ...
  document.getElementById('recTable').innerHTML=`
    ...
    ${scored.map(s=>{
      ...
      return`<div class="rec-row ${buyRow}" data-ticker="${s.t}">
        ...
        <span><span class="sig-pill ${pillCls(s.signal)}">${s.signal}</span>${(s.sizeTier&&s.sizeTier!=='—'&&s.sizeTier!=='Avoid')?`<span title="Suggested relative position size — guidance, not advice" style="font-size:8px;padding:1px 4px;margin-left:4px;border-radius:3px;background:rgba(255,255,255,0.06);color:var(--text3);vertical-align:middle">${s.sizeTier}</span>`:''}${s.signalStability==='Borderline'?`<span title="Near a signal threshold — may flip on noise" style="color:var(--amber);margin-left:3px;font-weight:700" >~</span>`:''}</span>
      </div>`;
    }).join('')}`;
...
function openDetail(ticker){
  ...
  document.getElementById('dpContent').innerHTML=`
    ...
    <div class="discipline-box" style="border:1px solid var(--line);border-radius:8px;padding:10px 12px;margin-bottom:1.1rem;background:rgba(255,255,255,0.02)">
      ...
      <div style="font-size:9px;color:var(--text3);margin-top:6px">Relative guidance only — not financial advice or a specific allocation.</div>
    </div>`:''}
